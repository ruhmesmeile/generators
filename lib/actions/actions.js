...
/* @TODO: find grunt 0.4.x substitute for this:
...
actions.directory = function directory(source, destination, noProcess) {
  ...
  var list = grunt.file.expand({ filter: 'isFile', dot: true }, path.join(root, '**'));
...
actions.remote = function(username, repo, branch, cb) {
  ...
  var url = 'http://nodeload.github.com/' + [username, repo, 'tarball', branch].join('/');
  ...
  function done(err) {
    ...
    var files = grunt.file.expand({ filter: 'isFile' }, path.join(cache, '**')).map(function(filepath) {
