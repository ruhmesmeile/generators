...
function Base(args, options, config) {
  ...
  // the generator config overrides in grunt config.
  this.fallbacks = this.config.generators || this.config.generator || {};

  this.description = '';

  // async noop (redefined per run)
  this.async = function() {
    return function() {};
  };

  // setup default options
  this.options = _.defaults(options || {}, this.fallbacks, {
...
Base.prototype.option = function option(name, config) {
  ...
  // update existing hash options, setting up the default value if not in the
  // hash yet
...
Base.prototype.optionsHelp = function optionsHelp() {
  ...
  out += rows.map(function(row) {
    var defaults = row[4] ? '\n' + pad + '# ' + row[4] : '';
    return this.log.table(widths, row) + defaults;
  }.bind(this)).join('\n');
